...
async function handler(req, res) {
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      // O log da Vercel tem engolido estas mensagens, então gravamos o erro
      // no próprio banco, que é onde conseguimos ler com segurança.
      await supabase.from('whatsapp_messages')
        .update({
          status: 'failed',
          payload: {
            ...
            erro_detalhe: e.detail ? JSON.stringify(e.detail) : null,
            erro_status: e.status || null,
            stack: (e.stack || '').split('\n').slice(0, 4).join(' | ')
          }
        })
        .eq('wa_message_id', job.waMessageId)
        .then(() => {}, (err) => console.error('Falha ao gravar o erro:', err.message));
